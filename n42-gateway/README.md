# N42 Gateway helm chart

[N42 Gateway](https://github.com/n42-gateway/n42-gateway) - Ingress and Gateway API controller built on HAProxy®

## Introduction

This chart bootstraps a N42 Gateway deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
To install the latest stable version with the release name `n42` in the current namespace and using default configurations:
...
* All resources will be created in the current namespace. Add `--create-namespace --namespace=<ns>` command-line options to install N42 Gateway in another one
* The default configuration installs N42 Gateway as a deployment, add `--set controller.kind=DaemonSet` command-line option to install as a DaemonSet
...
* If the release name is `"n42"` or contains `"n42-gateway"`, the resource names will not add the release name prefix and will have a shorter name
* Chart versions are in sync with minor N42 Gateway versions, so:
    * Use eg `--version '~0.16'` command-line option to install the latest `v0.16` release of N42 Gateway
    * See available chart and controller versions with `helm search repo n42 -l`
    * Add `--devel` command-line option to enable or list non stable versions as well

## Upgrading to a new Chart version

To upgrade the release `n42-gateway` to the latest stable version:

```console
$ helm repo update n42
$ helm upgrade n42 n42/n42-gateway
```

**Upgrade warning:**: chart since v0.17 changed its name to N42 Gateway, see in the [v0.17 release notes](https://github.com/n42-gateway/n42-gateway/blob/master/CHANGELOG/CHANGELOG-v0.17.md#upgrade-notes) how to preserve backward compatibility.
...
To uninstall/delete the `n42` release:
...
To run the N42 Gateway in dual-stack mode you will need a Kubernetes cluster that supports dual-stack services (v1.20+ with the `IPv6DualStack` feature gate enabled), and make the following configuration changes:
...
Note that in IPv6-only mode, logging [does not work](https://github.com/n42-gateway/charts/issues/15) by default. Enable the external/sidecar HAProxy setting and leave logs disabled to work around this:
...
The following table lists the configurable parameters of the N42 Gateway chart and their default values.
...
`controller.extraEnvs` | extra environment variables for  N42 Gateway | `[]`
`controller.extraVolumes` | extra volumes for N42 Gateway | `[]`
`controller.extraVolumeMounts` | extra volume mounts for N42 Gateway | `[]`
`controller.extraContainers` | extra containers to N42 Gateway | `[]`
`controller.initContainers` | extra init containers to N42 Gateway | `[]`
`controller.template` | custom template for N42 Gateway | `{}`
`controller.templateFile` | custom haproxy template path for N42 Gateway | `""`
...
`controller.ingressClassResource.controllerClass` | customizes the [controller name](https://n42-gateway.github.io/docs/configuration/command-line/#ingress-class) | `''`
...
`controller.podAnnotations` | Annotations for N42 Gateway pod | `{}`
`controller.labels` | Labels to be added to DaemonSet/Deployment definitions | `{}`
`controller.podLabels` | Labels for N42 Gateway pod | `{}`
`controller.podAffinity` | Add affinity to the controller pods to control scheduling | `{}`
`controller.podSecurityContext` | Security context settings for N42 Gateway pod | `{}`
`controller.priorityClassName` | Priority Class to be used | ``
`controller.securityContext` | Security context settings for N42 Gateway pod or container, see `controller.legacySecurityContext` | `{}`
`controller.securityContextInitContainer` | Security context settings for the init container | `{}`
`controller.config` | additional N42 Gateway [ConfigMap entries](https://n42-gateway.github.io/docs/configuration/keys/) | `{}`
...
`controller.tcp` | TCP [service ConfigMap](https://n42-gateway.github.io/docs/configuration/command-line/#tcp-services-configmap): `<port>: <namespace>/<servicename>:<portnumber>[:[<in-proxy>][:<out-proxy>]]` | `{}`
...
`controller.stats.port` | The port number N42 Gateway use for haproxy statistics | `1936`
`controller.stats.hostPort` | The host port number N42 Gateway use for haproxy statistics | `~`
